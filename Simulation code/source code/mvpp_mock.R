...
dir2 <- "C:/Users/20202943/Documents/BAM BEP/source codes/copulas included/"
source(paste0(dir2, "similarity_criterion.R"))
...
#libraries
library(copula)
mvpp <- function(method, variant = NULL, ensfc, ensfc_init, obs, obs_init, postproc_out, EMOS_sample = NULL, ECC_out = NULL, timeWindow, uvpp = NULL, MVPP_sample = NULL) {
  set.seed(2025)
  ...
  if (any(match(c(
    ...
    for (nn in 1:n) {
      ...
      if (any(match(c(
        "Clayton", "Frank", "Gumbel", "GOF"), method, nomatch = 0) != 0)) {
  ...
  if (method == "dECC") {
    if (is.null(EMOS_sample)) {
      EMOS_sample <- mvpp(
        method = "EMOS", variant = variant, postproc_out = postproc_out, # variant = variant terug zetten!
        ensfc = ensfc, ensfc_init = ensfc_init,
        obs = obs, obs_init = obs_init
      )
