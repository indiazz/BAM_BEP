...
run_setting_artificial <- function(obsmodel, fcmodel, nout, ninit, nmembers, MCrep, rand_rep, progress_ind = FALSE, compute_crps, ...){
  ecc_m <- NULL
  d <- list(...)$d
...
run_wrapper <- function(runID){
  sink(file = paste0(Rout_dir, "setting6mock_", runID, ".Rout"))
  tau <- runif(1, 0, 1)
  ...
  res <- run_setting_artificial(obsmodel = 7, fcmodel = 7, nout = evalDays, ninit = trainingDays, 
                      ...
                      d = input_par$d[runID])
  savename <- paste0(Rdata_dir, "res_setting6mcok_", runID, ".Rdata")
